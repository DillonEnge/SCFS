...
class TraitTreeModifier:
    ...
    def get_trait_tree(self):
        trait_tree = {}
        with open('trait_trees/' + self.name + '_trait_tree.json') as trait_tree_file:
...
def iterate_value_generation(trait_tree, traits, trait_tree_modifier = {}, subtraits = [], iteration = -1, name = 'null'):
    ...
    for i in range(len(selected_traits)):
        ...
        if len(subtraits) == 0:
            ...
            iterate_value_generation(trait_tree, traits, trait_tree_modifier=trait_tree_modifier, subtraits=get_and_shuffle_trait_tree(trait_tree['traits'][traits[i]]['subtraits']), iteration=i, name=name)
        else:
    ...
    trait_tree_modifier.write_trait_tree(trait_tree)
